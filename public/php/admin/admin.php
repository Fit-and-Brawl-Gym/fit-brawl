...
if ($conn->query("SHOW TABLES LIKE 'subscriptions'")->num_rows) {
    ...
    $has_request_status =
        $conn->query("SHOW COLUMNS FROM user_memberships LIKE 'request_status'")
            ->num_rows > 0;
    $has_membership_status =
        $conn->query(
            "SHOW COLUMNS FROM user_memberships LIKE 'membership_status'",
        )->num_rows > 0;
    $has_status =
        $conn->query("SHOW COLUMNS FROM user_memberships LIKE 'status'")
            ->num_rows > 0;

    if ($has_request_status) {
        $result = $conn->query(
            "SELECT COUNT(*) AS total FROM user_memberships WHERE request_status = 'pending'",
        );
        if ($result) {
            $pendingSubs = $result->fetch_assoc()["total"];
        }
        ...
        $result = $conn->query(
            "SELECT COUNT(*) AS total FROM user_memberships WHERE status IN ('Pending','pending')",
        );
        if ($result) {
            $pendingSubs = $result->fetch_assoc()["total"];
        }
        ...
        $result = $conn->query(
            "SELECT COUNT(*) AS total FROM user_memberships WHERE membership_status IS NULL AND date_submitted IS NOT NULL AND date_approved IS NULL",
        );
        if ($result) {
            $pendingSubs = $result->fetch_assoc()["total"];
        }
...
<main>
  ...
  <div class="admin-main">
    ...
    <section class="cards">
      ...
      $unreadContacts = 0;
      $unread_query = $conn->query(
          "SELECT COUNT(*) as count FROM contact WHERE status = 'unread' AND (archived = 0 OR archived IS NULL) AND deleted_at IS NULL",
      );
      if ($unread_query && ($unread_row = $unread_query->fetch_assoc())) {
          $unreadContacts = $unread_row["count"];
      }
      ?>
      <div class="card card-contacts <?= $unreadContacts > 0
          ? "has-unread"
          : "" ?>">
        <div class="card-icon">
        ...
        <tbody>
          ...
          if (!empty($activities)):
              foreach ($activities as $activity):
                  ...
                  $iconData = ActivityLogger::getActivityIcon(
