...
try {
  ...
  if ($membership_result && $membership_result->num_rows > 0) {
    ...
    if ($booking_date_obj > $max_booking_date) {
      ...
      echo json_encode([
        'success' => false,
        'message' =>
          "Cannot book beyond your membership expiration. Your {$plan_name} plan expires on " .
          $end_date_obj->format('F d, Y') .
          ' (booking allowed until ' .
          $max_booking_date->format('F d, Y') .
          ' with grace period). Please visit the gym to renew or upgrade your membership.',
  ...
  $trainer_stmt = $conn->prepare(
    'SELECT name, email FROM trainers WHERE id = ?',
  );
  ...
  try {
    ...
    // Log activity
    // FIX: Use real username from DB instead of missing/incorrect session key
    $target_username = $member_data['username'] ?? 'User';
    $log_details = "Booked {$class_type} session with {$trainer_name} on {$booking_date} ({$session_time}: {$session_hours})";

    // Log the activity with the specific username as the "Target User"
    ActivityLogger::log(
      'session_booked',
      $target_username,
      $booking_id,
    ...
    $week_start = date(
      'Y-m-d',
      strtotime($booking_date . ' -' . $day_of_week . ' days'),
    );
    ...
    try {
      if (function_exists('sendTrainerBookingNotification')) {
        ...
        if (!$email_sent) {
          error_log(
            "Failed to send booking notification to trainer: {$trainer_data['email']}",
          );
        }
      } else {
        // Mail function not defined in mail_config.php
        error_log(
          'sendTrainerBookingNotification() not defined. Check mail_config.php',
        );
      }
