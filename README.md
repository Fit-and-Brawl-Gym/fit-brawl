...
- � **AES-256-GCM Encryption**: Military-grade encryption for sensitive data at rest (emails, phone numbers, personal information)
...
#### **Step 2: Setup AES-256 Encryption** ⭐ **NEW - REQUIRED**

**For Team Members:** After pulling the code, run the auto-setup script:
...
- ✅ Add `email_encrypted` column to database
- ✅ Verify encryption is working
- ✅ Show you the results (takes ~2 minutes)

**What it does:**
The script sets up AES-256-GCM encryption for protecting sensitive data (emails) at rest. It's safe to run multiple times and won't break anything!

**Manual Setup** (if preferred):
```bash
# Add encryption column
mysql -u root -p fit_brawl < database/migrations/add_encryption.sql

# Verify setup
php test_encryption.php
```

📖 **For more details, see:** `TEAM-SETUP.md` or `docs/security/AES-ENCRYPTION-GUIDE.md`
...
# AES Encryption Key (auto-generated by setup_encryption.php)
ENCRYPTION_KEY=your_32_byte_hex_encryption_key_here
```

**Note:** If you ran `setup_encryption.php`, the encryption key is already configured in `includes/config.php`. For production, move it to `.env` instead.
